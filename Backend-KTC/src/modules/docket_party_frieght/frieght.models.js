...
export const addPartyFrieght = async (newDocketFrieghtData, connection) => {
    try {
        ...
        const { query, values } = buildInsertQuery("party_freight", newDocketFrieghtData)
        const [result] = connection
            ? await connection.query(query, values)
            : await connectionPool.query(query, values);

        if (result.affectedRows !== 1) {
            throw new AppError("Failed to insert docket Party Frieghts.", 500);
        }

        return {
            message: `Docket Party Frieghts Added successfully`,
