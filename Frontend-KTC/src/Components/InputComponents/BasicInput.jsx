...
const BasicInput = memo(
  ({ name, label = "Default Label", type = "text", upperCase = false, isRegister=true,  ...rest }) => {
    const labelText =
...
const registerProps =  isRegister
    ? register(name, {
        setValueAs: (value) =>
        ...
        <div className="relative">
          <input
           ...
           {...registerProps}
            {...rest}
             onBlur={(e) => {
                          registerProps.onBlur?.(e); // RHF
                          rest.onBlur?.(e);          // Custom
                        }}
            
            className={` ${upperCase ? "uppercase" : ""}  no-spinner peer w-full border rounded-lg px-3 pt-4 pb-2 text-gray-800 focus:outline-none transition-all border-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
          />
